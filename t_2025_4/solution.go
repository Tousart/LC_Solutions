...
func drain(tanks [][3]int) int {
	...
	// right
	right[n-1] = min(tanks[n-1][2], tanks[n-2][1])
	for i := n - 2; i > 0; i-- {
		if tanks[i-1][1] > tanks[i][2] {
			right[i] = min(tanks[i-1][1], tanks[i][2]+right[i+1])
		} else {
			right[i] = tanks[i-1][1]
		}
	}
	...
	// left
	left[0] = min(tanks[0][2], tanks[1][0])
	for i := 1; i > n-1; i-- {
		if tanks[i+1][0] > tanks[i][2] {
			left[i] = min(tanks[i+1][0], tanks[i][2]+left[i-1])
		} else {
			left[i] = tanks[i+1][0]
		}
	}
